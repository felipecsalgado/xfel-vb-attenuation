...
:G4UImessenger(), 
 ...
 fMaterCmd(0), fSizeXYCmd(0), fSizeZCmd(0),
 fKaptonThickCmd(0), fAirThickCmd(0), fUpdateCmd(0)
{ 
  ...
  fKaptonThickCmd = new G4UIcmdWithADoubleAndUnit("/testem/det/setKaptonThickness",this);
  fKaptonThickCmd->SetGuidance("Set Kapton window thickness");
  fKaptonThickCmd->SetParameterName("Size",false);
  fKaptonThickCmd->SetRange("Size>=0.");
  fKaptonThickCmd->SetUnitCategory("Length");
  fKaptonThickCmd->AvailableForStates(G4State_PreInit,G4State_Idle);

  fAirThickCmd = new G4UIcmdWithADoubleAndUnit("/testem/det/setAirThickness",this);
  fAirThickCmd->SetGuidance("Set Air volume thickness");
  fAirThickCmd->SetParameterName("Size",false);
  fAirThickCmd->SetRange("Size>0.");
  fAirThickCmd->SetUnitCategory("Length");
  fAirThickCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
...
{ 
  ...
  if( command == fKaptonThickCmd )
   { fDetector->SetKaptonThickness(fKaptonThickCmd->GetNewDoubleValue(newValue));}

  if( command == fAirThickCmd )
   { fDetector->SetAirThickness(fAirThickCmd->GetNewDoubleValue(newValue));}
