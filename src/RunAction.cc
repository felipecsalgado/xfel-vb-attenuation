...
void RunAction::FillData(const G4ParticleDefinition* particle,
                         ...
                         G4double phi, G4double longitudinalPolarization, G4double zpos,
...
void RunAction::BookHisto()
{
  ...
  G4int ih2;
  // gamma phi vrs cos(theta)
  ih2 = fAnalysisManager->
  CreateH2("h2d", "Gamma phi vrs cos(theta)", 100,-180*deg, 180*deg, 100, -1, 1);
  G4cout << "2D histogram id = " << ih2 << G4endl;
  fAnalysisManager->SetH2Activation(1, true); //1
  
  ih2 = fAnalysisManager->
  CreateH2("h2d", "Electron phi vrs cos(theta)", 100,-180*deg, 180*deg, 100, -1, 1);
  G4cout << "2D histogram id = " << ih2 << G4endl;
  fAnalysisManager->SetH2Activation(1, true); //2
  
  ih2 = fAnalysisManager->
  CreateH2("h2d", "Positron phi vrs cos(theta)", 100,-180*deg, 180*deg, 100, -1, 1);
  G4cout << "2D histogram id = " << ih2 << G4endl;
  fAnalysisManager->SetH2Activation(1, true); //2
...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  if (fPrimary) {
    ...
    G4cout << "\n The run consists of " << NbOfEvents << " events through "
           << G4BestUnit(fDetector->GetBoxSizeZ(),"Length") << " of "
           << material->GetName() << " (density: " 
           << G4BestUnit(density,"Volumic Mass") << ")" << G4endl;
  }
